package Dynamic_Programming;

import java.util.Scanner;

public class Cell_Mitosis {
        public static void main(String args[]) {
            Scanner sc=new Scanner(System.in);
            int n=sc.nextInt();
            int x=sc.nextInt();
            int y=sc.nextInt();
            int z=sc.nextInt();
            System.out.println(mincost(x,y,z,n));
        }
        public static long mincost(int x,int y,int z,int n){
            long dp[]=new long[n+1];
            for(int i=2;i<dp.length;i++){
                if(i%2==0)dp[i]=Math.min(dp[i/2]+x,dp[i-1]+y);
                else dp[i]=Math.min(dp[(i+1)/2]+x+z,dp[i-1]+y);
            }
            return dp[n];
        }

}
//        Input
//        5
//        2 1 3
//        Output
//        4
//        Explanation
//        Initial number of cell is 1. Applying 2nd operations four times will give 5 cells in the container , and its total cost will be 1x4 = 4.
//        One other way to do is - first adding a single cell with operation 2nd ,then doubling the cells , and then again adding one more cell , with cost = 1+2+1 =4.
